...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams t FULL JOIN games g1 ON t.team_id = g1.winner_id FULL JOIN games g2 ON t.team_id = g2.opponent_id WHERE g1.round = 'Eighth-Final' AND g1.year = 2014 OR g2.round =
'Eighth-Final' AND g2.year = 2014 ORDER BY name")"
...
echo "$($PSQL "SELECT year, name FROM teams RIGHT JOIN games ON teams.team_id = games.winner_id WHERE round
= 'Final' ORDER BY year")"
